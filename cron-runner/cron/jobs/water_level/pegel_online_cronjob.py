...
class PegelOnlineCronjob(CronjobBase):
    ...
    def __init__(self):
        ...
        self.KONSTANZ_RHEIN = (PegelOnline.Station.KONSTANZ_RHEIN, 3329, "Konstanz Rhein")
        self.KONSTANZ_BODENSEE = (PegelOnline.Station.KONSTANZ_BODENSEE, 906, "Konstanz Bodensee")
    ...
    def start(self, local_dt: datetime) -> bool:
        try:
            df_rhein = self.pegel_online.get_water_level_measurements(PegelOnline.Period.last_24_hours, self.KONSTANZ_RHEIN[0])
            df_bodensee = self.pegel_online.get_water_level_measurements(PegelOnline.Period.last_24_hours, self.KONSTANZ_BODENSEE[0])
    ...
    def write_data_to_influxdb(self, df: pd.DataFrame, station_infos: tuple):
        with self.client.write_api(write_options=SYNCHRONOUS) as write_api:
            for index, row in df.iterrows():
                ...
                write_api.write(bucket=settings.influx.bucket, org="FogCast", record=point)